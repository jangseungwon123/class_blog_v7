...
public class JwtUtil {
    ...
    // 토근 만료 시간(1시간 = 1000 * 60 * 60)
    private static final long EXPIRATION_TIME = 1000 * 60 * 60;
    // 토근 주제 ( 이 애플리케이션을 식별하는 값)
    ...
    public static String create(User user) {
        ...
        String jwt = JWT.create()
                ...
                .withExpiresAt(expiresAt) // 토큰만료시간 설정
                .withClaim("id", user.getId())
                .withClaim("username", user.getUsername())
                .withClaim("email", user.getEmail())
                .withIssuedAt(new Date()) // 토근이 언제 발급되었는지 알려줌
                .sign(Algorithm.HMAC512(SECRET_KEY)); // 서명에 필요함 ,특정한 알고리즘으로 보안 HMAC512이것으로
    ...
    public static SessionUser verify(String jwt) {
        ...
        // JWT 디코딩
        DecodedJWT decodedJWT = JWT.require(Algorithm.HMAC512(SECRET_KEY))// 필수값
                .withSubject(SUBJECT)
                .build().verify(jwt); // 이걸 넣어야 풀림

        // 풀린 토큰 정보에서 사용자 정보 추출 (민감한 정보는 넣지 않는다)
        Long id = decodedJWT.getClaim("id").asLong(); // .asLong()형변환
    ...
    public static Long getUserId(String jwt) {
        DecodedJWT decodedJWT = JWT.require(Algorithm.HMAC512(SECRET_KEY)) // 조건의 검사하고 아니면 예외
                .withSubject(SUBJECT)
                .build()
                .verify(jwt);
