 ...
 * POST, PUT 요청에서 발생하는 유효성 검사 유류를 자동으로 처리
 */
@Aspect
@Component
@Slf4j
public class MyValidationHandler {
     ...
     * 2. 메서드의 매개 변수중 Errors 객체 찾기
     * 3. 유효성 검사 오류가 있으면 Exception400 던지기
     */
    ...
    public void validationCheck(JoinPoint joinPoint) {
        ...
        // JoinPoint 에서 메서드의 모든 매개 변수 가져오기
        Object[] args = joinPoint.getArgs();

        // 매개변수를 하나씩 검사해서 Errors 객체를 찾기 (어쩔수 없이 코드가 로우)
        for (Object arg : args) {
            // instanceof : 객체가 특정 타입인지 확인하는 연산자
            if (arg instanceof Errors) {
                log.debug("Errors 객체 발견 - 유효성 검사 진행");
                Errors errors = (Errors)arg;// 안정적 다운캐스팅가능

                if (errors.hasErrors()){
                    //오류가 있다면
                    log.warn("유효성 검사 오류 - 오류 개수 :{}",errors.getErrorCount());
