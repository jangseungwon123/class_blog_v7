...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
       ...
       log.debug("=== JWT 인증 인터셉터 시작 ===");
        ...
        // Bearer + 공백 adsfasdfas.asdfasfsafaf.afsdfsadfsadf
        if (jwt == null || jwt.startsWith("Bearer ")) {
            throw new Exception401("JWT 토큰을 전달해주세요");
        }
        jwt = jwt.replace("Bearer ", ""); // 이것을 쓰면adsfasdfas.asdfasfsafaf.afsdfsadfsadf이것만 남음

        try{
            SessionUser sessionUser = JwtUtil.verify(jwt);

            // TODO 구분 중요!!(임시 세션 사용 )
            //HttpSession session = request.getSession(); // JSSION="asdas123123"
            //session.setAttribute(Define.SESSION_USER,sessionUser);

            //JWT는 stateless 개면을 지키기 위해서 나옴(모바일 쿠기에 접근 못함)
            // request.setAttribute는 요청 단위를 데이터를 저장하고 소멸함
            // 즉, 해당 데이터는 요청이 처리된 후 사라지며, 서버에 세션메모리에 저장되지 않습니다.
